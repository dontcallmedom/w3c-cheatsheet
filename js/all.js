...
if(source[keyword].synonym){keyword.push(source[keyword].synonym)
...
if(source[keyword].synonym){keywordsMatch[source[keyword].synonym][infoset][propertytype].push(source[keyword]["d"][k])
...
}function A(H){if(H===null){return 
}var F=H.selectValue;
window.location.hash="#search,"+escape(F);
clearLookUp();
var E=0;
for(var G in keywordsMatch[F]){for(var I in keywordsMatch[F][G]){E=E+1;
C(F,G,I)
...
if(window.location.hash){if(window.location.hash.substring(0,5)==="#inf,"){B(window.location.hash.substring(1))
}else{if(window.location.hash.substring(0,8)==="#search,"){D("#search").val(window.location.hash.substring(8));
D("#search").get(0).autocompleter.findValue()
}}}});