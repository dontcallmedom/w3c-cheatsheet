...
}}function B(s,j){var k=X.get(0);
if(k.createTextRange){var i=k.createTextRange();
i.collapse(true);
i.moveStart("character",s);
i.moveEnd("character",j);
i.select()
}else{if(k.setSelectionRange){k.setSelectionRange(s,j)
}else{if(k.selectionStart){k.selectionStart=s;
k.selectionEnd=j
}}}k.focus()
...
}}function F(u){if(!u){return null
}var j=[];
var s=u.split(T.lineSeparator);
for(var k=0;
k<s.length;
k++){var v=$.trim(s[k]);
if(v){j[j.length]=v.split(T.cellSeparator)
}}return j
}function l(v,k){var y=document.createElement("ul");
var x=v.length;
if((T.maxItemsToShow>0)&&(T.maxItemsToShow<x)){x=T.maxItemsToShow
}for(var w=0;
w<x;
w++){var AA=v[w];
if(!AA){continue
}var z=document.createElement("li");
if(T.formatItem){z.innerHTML=T.formatItem(AA,w,x);
z.selectValue=AA[0]
}else{var AB=AA[0];
AB=AB.substring(0,AB.indexOf(k))+"<strong>"+k+"</strong>"+AB.substring(AB.indexOf(k)+k.length);
z.innerHTML=AB;
z.selectValue=AA[0]
}var s=null;
if(AA.length>1){s=[];
...
y.appendChild(z);
$(z).hover(function(){$("li",y).removeClass("ac_over");
$(this).addClass("ac_over");
h=$("li",y).indexOf($(this).get(0))
...
if(i){G(j,i)
}else{if((typeof T.url=="string")&&(T.url.length>0)){$.get(N(j),function(k){k=F(k);
E(j,k);
G(j,k)
})
}else{X.removeClass(T.loadingClass)
}}}function N(s){var j=T.url+"?q="+encodeURI(s);
for(var k in T.extraParams){j+="&"+k+"="+encodeURI(T.extraParams[k])
}return j
}function p(k){if(!k){return null
}if(J.data[k]){return J.data[k]
}if(T.matchSubset){for(var w=k.length-1;
w>=T.minChars;
w--){var z=k.substr(0,w);
var y=J.data[z];
if(y){var AA=[];
var u=[];
for(var v=0;
...
v++){var AB=y[v];
var s=AB[0];
if(H(s,k)){AA[AA.length]=AB
}else{if(T.matchContains&&Z(s,k)){u[u.length]=AB
}}}return AA.concat(u)
}}}return null
}function H(j,i){return Z(j,i,true)
}function Z(v,u,j){if(j===null){j=!T.matchContains
}if(!T.matchCase){v=v.toLowerCase()
}var k=v.indexOf(u);
if(k==-1){return false
}return k==0||!j
...
function f(y,x){if(x){X.removeClass(T.loadingClass)
}var v=(x)?x.length:0;
var s=null;
for(var w=0;
w<v;
w++){var z=x[w];
if(z[0].toLowerCase()==y.toLowerCase()){s=document.createElement("li");
if(T.formatItem){s.innerHTML=T.formatItem(z,w,v);
s.selectValue=z[0]
}else{s.innerHTML=z[0];
s.selectValue=z[0]
}var k=null;
if(z.length>1){k=[];
for(var u=1;
...
}}if(T.onFindValue){setTimeout(function(){T.onFindValue(s)
},1)
...
}function Y(j){var k=j.offsetLeft||0;
var i=j.offsetTop||0;
...
function display_keyword_data(B,E,F){if(E&&F){var D={};
D[E]={};
D[E][F]=B[E][F];
B=D
}var A=-1;
for(var C in B){for(var G in B[C]){show_keyword(B,C,G);
A=A+1
...
}function load_keyword_data(A,B,C){if(inMemory&&keywordsMatch[A]&&(!B||(keywordsMatch[A][B]&&keywordsMatch[A][B][C]&&keywordsMatch[A][B][C]!==1))){display_keyword_data(keywordsMatch[A],B,C)
}else{$.getJSON("data/json/"+escape(A).toLowerCase()+".js",function(D){display_keyword_data(D,B,C)
})
}}function show_keyword(N,J,O){if(N[J][O]===null){return false
}for(var X in N[J][O]){var G=keywordSources[J][O];
...
}function load_anchor(C){if(C===null){return false
...
if(B&&D&&E&&keywordSources[D]&&keywordSources[D][E]&&keywordsMatch[B]&&keywordsMatch[B][D]&&keywordsMatch[B][D][E]){clearLookUp();
$("#search").val("");
if(load_keyword_data(B,D,E)){return true
}}return false
...
}jQuery(document).ready(function(C){C("#content").tabs();
C("#content").tabs("paging");
C("#content").bind("tabsshow",function(F,G){window.location.hash=G.tab.hash
});
C(".accordion").accordion({header:"div >h3",active:false,autoHeight:false});
makeReplacingAccordion(C(".accordion"));
C("body").css("display","block");
if(getCookie("alreadyLaunched")===""&&startWithDonate){E();
setCookie("alreadyLaunched","true",2000)
}C("#openabout").click(E);
function D(){C("#about").css("display","none");
return false
}function E(){if(C("#about").css("display")==="block"){D()
}else{C("#about").css("display","block");
C("#closeabout").click(D);
C("#closeabout2").click(D)
}return false
}function B(){if(hashHistory.length>0){C("<a class='internal back' onclick='hashHistory.pop();return true;'></a>").attr("href",hashHistory[hashHistory.length-1]).text("back").appendTo(C("<p></p>")).appendTo(C("#details"))
}}function A(G){if(G===null){return 
}var F=G.selectValue;
window.location.hash="#search,"+escape(F);
clearLookUp();
load_keyword_data(F)
}C("a.internal").live("click",function(){if(load_anchor(C(this).attr("href").split("#")[1])&&!C(this).hasClass("back")){hashHistory.push(window.location.hash);
B()
}});
C("#search").autocompleteArray(keywords,{onItemSelect:A,onFindValue:A,autoFill:false,selectFirst:true,delay:40,maxItemsToShow:10,matchContains:true,matchSubset:true});
C("#search").change(function(){clearLookUp();
if(C("#search").val()){if(!C("#details_clear").length){C("#search").after("<a href='#' class='ui-icon-close' id='details_clear' title='Clear search'>x</a>");
C("#details_clear").click(function(){clearLookUp();
C("#search").val("").change()
})
}}else{C("#details_clear").replaceWith("")
}});
if(window.location.hash){if(window.location.hash.substring(0,5)==="#inf,"||window.location.hash.substring(0,8)==="#search,"){load_anchor(window.location.hash.substring(1))
}}});