...
}}}}}function show_keyword(W,J,N){if(W===null){return false
}var G=keywordSources[J][N];
var R=$("<div></div>").addClass("context");
$("<code></code>").text(W).appendTo($("<h2></h2>").text(G+" ").appendTo(R));
var C=$("<div></div>").appendTo(R);
for(var Q in keywordsMatch[W][J][N]){var D=keywordsMatch[W][J][N][Q];
var V=$("<dl></dl>");
for(var K in D){var S=$("<dt></dt>").appendTo(V);
var O=S;
if(D[K].u){var H=D[K].u;
if(H.substring(0,1)==="/"){H="http://www.w3.org"+H
}O=$("<a></a>").attr("href",H).appendTo(S)
}O.text(dictionary[K]);
if(D[K]["p"]&&D[K]["p"].length>0){var A=true;
if(D[K]["p"].length===1||D[K].l==="inline"){A=false
}var T=$("<dd></dd>");
if(A){if(D[K].l==="block"){T=$("<ul></ul>").appendTo(T)
...
E=true
}}if(F.t instanceof Array){if(!E){for(var U in F.t){var M=F.t[U];
if(M.y&&M.i&&M.t){$("<a class='internal'></a>").attr("href","#inf,"+escape(M.i)+","+escape(M.y)+","+escape(M.t)).text(M.t).appendTo(B)
}else{var P=document.createTextNode(M);
B.get(0).appendChild(P)
}}}else{B.text(F.t.join(""))
}}else{B.text(F.t)
}if(!A&&L<D[K]["p"].length-1){T.append(", ")
}}T.appendTo(V)
}}V.appendTo(C)
}R.appendTo($("#details"));
return true
...
$("#search").val("");
if(show_keyword(B,D,E)){$("#details").accordion({header:"div>h2",autoHeight:false});
return true
}}return false
...
show_keyword(G,H,J)
}}if(F===1){C("#details").accordion({header:"div>h2",autoHeight:false})
}else{C("#details").accordion({header:"div>h2",autoHeight:false,active:false})
}makeReplacingAccordion(C("#details"))
