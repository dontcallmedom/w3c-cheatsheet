...
if(jQuery.isFunction(customJsonp)){customJsonp(tmp)
}else{window[jsonp]=undefined;
try{delete window[jsonp]
}catch(jsonpError){}}if(head){head.removeChild(script)
}}
}if(s.dataType==="script"&&s.cache===null){s.cache=false
}if(s.cache===false&&type==="GET"){var ts=jQuery.now();
...
xhr.abort=function(){if(xhr&&oldAbort.call){oldAbort.call(xhr)
...
jQuery.fn.extend({show:function(speed,easing,callback){if(speed||speed===0){return this.animate(genFx("show",3),speed,easing,callback)
}else{for(var i=0,j=this.length;
i<j;
i++){if(!jQuery.data(this[i],"olddisplay")&&this[i].style.display==="none"){this[i].style.display=""
...
return obj
}jQuery.each({slideDown:genFx("show",1),slideUp:genFx("hide",1),slideToggle:genFx("toggle",1),fadeIn:{opacity:"show"},fadeOut:{opacity:"hide"}},function(name,props){jQuery.fn[name]=function(speed,easing,callback){return this.animate(props,speed,easing,callback)
}
});
...
},custom:function(from,to,unit){this.startTime=jQuery.now();
this.start=from;
this.end=to;
...
if(!C.ui.version){C.extend(C.ui,{version:"1.8.5",keyCode:{ALT:18,BACKSPACE:8,CAPS_LOCK:20,COMMA:188,COMMAND:91,COMMAND_LEFT:91,COMMAND_RIGHT:93,CONTROL:17,DELETE:46,DOWN:40,END:35,ENTER:13,ESCAPE:27,HOME:36,INSERT:45,LEFT:37,MENU:93,NUMPAD_ADD:107,NUMPAD_DECIMAL:110,NUMPAD_DIVIDE:111,NUMPAD_ENTER:108,NUMPAD_MULTIPLY:106,NUMPAD_SUBTRACT:109,PAGE_DOWN:34,PAGE_UP:33,PERIOD:190,RIGHT:39,SHIFT:16,SPACE:32,TAB:9,UP:38,WINDOWS:91}});
...
if(D==="absolute"||D==="relative"||D==="fixed"){D=parseInt(E.css("zIndex"));
...
}return this.each(function(){C.style(this,G,I(this,J)+"px")
})
};
C.fn["outer"+D]=function(K,J){if(typeof K!=="number"){return F["outer"+D].call(this,K)
}return this.each(function(){C.style(this,G,I(this,K,true,J)+"px")
})
}
});
...
C(function(){var E=document.createElement("div"),D=document.body;
C.extend(E.style,{minHeight:"100px",height:"auto",padding:0,borderWidth:0});
C.support.minHeight=D.appendChild(E).offsetHeight===100;
...
if(I&&J.substring(0,1)==="_"){return G
}I?this.each(function(){var L=A.data(this,E);
if(!L){throw"cannot call methods on "+E+" prior to initialization; attempted to call method '"+J+"'"
}if(!A.isFunction(L[J])){throw"no such method '"+J+"' for "+E+" widget instance"
}var K=L[J].apply(L,H);
if(K!==L&&K!==D){G=K;
return false
...
this.element=A(G);
this.options=A.extend(true,{},this.options,A.metadata&&A.metadata.get(G)[this.widgetName],E);
var F=this;
...
},option:function(E,H){var G=E,F=this;
if(arguments.length===0){return A.extend({},F.options)
}if(typeof E==="string"){if(H===D){return this.options[E]
}G={};
G[E]=H
...
H=G.height();
I={top:0,left:0}
}else{if(M.scrollTo&&M.document){J=G.width();
H=G.height();
...
}}C.active=C._findActive(C.active||B.active).addClass("ui-state-default ui-state-active").toggleClass("ui-corner-all ui-corner-top");
...
A.extend(A.ui.accordion,{version:"1.8.5",animations:{slide:function(C,B){C=A.extend({easing:"swing",duration:300},C,B);
...
},_tabify:function(Q){function N(S,T){S.css("display","");
!F.support.opacity&&T.opacity&&S[0].style.removeAttribute("filter")
}var P=this,O=this.options,M=/^#.+/;
...
W.href=T
}if(M.test(T)){P.panels=P.panels.add(P._sanitizeSelector(T))
}else{if(T&&T!=="#"){F.data(W,"href.tabs",T);
F.data(W,"load.tabs",T.replace(/#.*$/,""));
T=P._tabId(W);
W.href="#"+T;
W=F("#"+T);
if(!W.length){W=F(O.panelTemplate).attr("id",T).addClass("ui-tabs-panel ui-widget-content ui-corner-bottom").insertAfter(P.panels[U-1]||P.list);
W.data("destroy.tabs",true)
}P.panels=P.panels.add(W)
}else{O.disabled.push(U)
}}});
if(Q){this.element.addClass("ui-tabs ui-widget ui-widget-content ui-corner-all");
...
if(typeof O.selected!=="number"&&O.cookie){O.selected=parseInt(P._cookie(),10)
}if(typeof O.selected!=="number"&&this.lis.filter(".ui-tabs-selected").length){O.selected=this.lis.index(this.lis.filter(".ui-tabs-selected"))
}O.selected=O.selected||(this.lis.length?0:-1)
}else{if(O.selected===null){O.selected=-1
}}O.selected=O.selected>=0&&this.anchors[O.selected]||O.selected<0?O.selected:0;
O.disabled=F.unique(O.disabled.concat(F.map(this.lis.filter(".ui-state-disabled"),function(S){return P.lis.index(S)
}))).sort();
F.inArray(O.selected,O.disabled)!=-1&&O.disabled.splice(F.inArray(O.selected,O.disabled),1);
this.panels.addClass("ui-tabs-hide");
this.lis.removeClass("ui-tabs-selected ui-state-active");
if(O.selected>=0&&this.anchors.length){this.panels.eq(O.selected).removeClass("ui-tabs-hide");
this.lis.eq(O.selected).addClass("ui-tabs-selected ui-state-active");
P.element.queue("tabs",function(){P._trigger("show",null,P._ui(P.anchors[O.selected],P.panels[O.selected]))
});
this.load(O.selected)
}F(window).bind("unload",function(){P.lis.add(P.anchors).unbind(".tabs");
...
for(var L;
L=this.lis[Q];
Q++){F(L)[F.inArray(Q,O.disabled)!=-1&&!F(L).hasClass("ui-tabs-selected")?"addClass":"removeClass"]("ui-state-disabled")
...
P._trigger("show",null,P._ui(S,T[0]))
})
}:function(S,T){F(S).closest("li").addClass("ui-tabs-selected ui-state-active");
T.removeClass("ui-tabs-hide");
P._trigger("show",null,P._ui(S,T[0]))
},R=J?function(S,T){T.animate(J,J.duration||"normal",function(){P.lis.removeClass("ui-tabs-selected ui-state-active");
T.addClass("ui-tabs-hide");
N(T,J);
P.element.dequeue("tabs")
})
}:function(S,T){P.lis.removeClass("ui-tabs-selected ui-state-active");
T.addClass("ui-tabs-hide");
P.element.dequeue("tabs")
};
this.anchors.bind(O.event+".tabs",function(){var U=this,V=F(U).closest("li"),T=P.panels.filter(":not(.ui-tabs-hide)"),S=F(P._sanitizeSelector(U.hash));
if(V.hasClass("ui-tabs-selected")&&!O.collapsible||V.hasClass("ui-state-disabled")||V.hasClass("ui-state-processing")||P.panels.filter(":animated").length||P._trigger("select",null,P._ui(this,S[0]))===false){this.blur();
return false
}O.selected=P.anchors.index(this);
P.abort();
if(O.collapsible){if(V.hasClass("ui-tabs-selected")){O.selected=-1;
O.cookie&&P._cookie(O.selected,O.cookie);
P.element.queue("tabs",function(){R(U,T)
}).dequeue("tabs");
this.blur();
return false
}else{if(!T.length){O.cookie&&P._cookie(O.selected,O.cookie);
P.element.queue("tabs",function(){G(U,S)
});
P.load(P.anchors.index(this));
this.blur();
return false
}}}O.cookie&&P._cookie(O.selected,O.cookie);
if(S.length){T.length&&P.element.queue("tabs",function(){R(U,T)
});
P.element.queue("tabs",function(){G(U,S)
});
P.load(P.anchors.index(this))
...
return this
},add:function(H,K,G){if(G===E){G=this.anchors.length
}var L=this,J=this.options;
K=F(J.tabTemplate.replace(/#\{href\}/g,H).replace(/#\{label\}/g,K));
H=!H.indexOf("#")?H.replace("#",""):this._tabId(F("a",K)[0]);
K.addClass("ui-state-default ui-corner-top").data("destroy.tabs",true);
var I=F("#"+H);
I.length||(I=F(J.panelTemplate).attr("id",H).data("destroy.tabs",true));
I.addClass("ui-tabs-panel ui-widget-content ui-corner-bottom ui-tabs-hide");
if(G>=this.lis.length){K.appendTo(this.list);
I.appendTo(this.list[0].parentNode)
}else{K.insertBefore(this.lis[G]);
I.insertBefore(this.panels[G])
}J.disabled=F.map(J.disabled,function(M){return M>=G?++M:M
});
this._tabify();
...
var I=this.options,G=this.lis.eq(H).remove(),J=this.panels.eq(H).remove();
if(G.hasClass("ui-tabs-selected")&&this.anchors.length>1){this.select(H+(H+1<this.anchors.length?1:-1))
}I.disabled=F.map(F.grep(I.disabled,function(K){return K!=H
}),function(K){return K>=H?--K:K
});
this._tabify();
this._trigger("remove",null,this._ui(G.find("a")[0],J[0]));
return this
...
return this
},load:function(H){H=this._getIndex(H);
var K=this,G=this.options,L=this.anchors.eq(H)[0],J=F.data(L,"load.tabs");
this.abort();
if(!J||this.element.queue("tabs").length!==0&&F.data(L,"cache.tabs")){this.element.dequeue("tabs")
}else{this.lis.eq(H).addClass("ui-state-processing");
if(G.spinner){var I=F("span",L);
I.data("label.tabs",I.html()).html(G.spinner)
}this.xhr=F.ajax(F.extend({},G.ajaxOptions,{url:J,success:function(N,O){F(K._sanitizeSelector(L.hash)).html(N);
K._cleanup();
G.cache&&F.data(L,"cache.tabs",true);
K._trigger("load",null,K._ui(K.anchors[H],K.panels[H]));
try{G.ajaxOptions.success(N,O)
}catch(M){}},error:function(N,O){K._cleanup();
K._trigger("load",null,K._ui(K.anchors[H],K.panels[H]));
try{G.ajaxOptions.error(N,O,H,L)
}catch(M){}}}));
...
F.extend(F.ui.tabs,{version:"1.8.5"});
F.extend(F.ui.tabs.prototype,{rotation:null,rotate:function(H,J){var G=this,K=this.options,I=G._rotate||(G._rotate=function(L){clearTimeout(G.rotation);
G.rotation=setTimeout(function(){var M=K.selected;
G.select(++M<G.anchors.length?M:0)
},H);
L&&L.stopPropagation()
});
J=G._unrotate||(G._unrotate=!J?function(L){L.clientX&&G.rotate(null)
...
jQuery.effects||function(I,H){function F(M){var L;
...
}if(/rgba\(0, 0, 0, 0\)/.exec(M)){return E.transparent
}return E[I.trim(M).toLowerCase()]
}function A(N,M){var L;
do{L=I.curCSS(N,M);
if(L!=""&&L!="transparent"||I.nodeName(N,"body")){break
}M="backgroundColor"
}while(N=N.parentNode);
return F(L)
}function D(){var P=document.defaultView?document.defaultView.getComputedStyle(this,null):this.currentStyle,M={},L,O;
if(P&&P.length&&P[0]&&P[P[0]]){for(var N=P.length;
N--;
){L=P[N];
if(typeof P[L]=="string"){O=L.replace(/\-(\w)/g,function(R,Q){return Q.toUpperCase()
});
M[O]=P[L]
}}}else{for(L in P){if(typeof P[L]==="string"){M[L]=P[L]
}}}return M
}function C(N){var M,L;
for(M in N){L=N[M];
if(L==null||I.isFunction(L)||M in K||/scrollbar/.test(M)||!/color/i.test(M)&&isNaN(parseFloat(L))){delete N[M]
}}return N
}function J(O,M){var L={_:0},N;
for(N in M){if(O[N]!=M[N]){L[N]=M[N]
}}return L
}function G(O,M,L,N){if(typeof O=="object"){N=M;
L=null;
M=O;
O=M.effect
}if(I.isFunction(M)){N=M;
L=null;
M={}
}if(typeof M=="number"||I.fx.speeds[M]){N=L;
L=M;
M={}
}if(I.isFunction(L)){N=L;
L=null
}M=M||{};
L=L||M.duration;
L=I.fx.off?0:typeof L=="number"?L:I.fx.speeds[L]||I.fx.speeds._default;
N=N||M.complete;
return[O,M,L,N]
}I.effects={};
I.each(["backgroundColor","borderBottomColor","borderLeftColor","borderRightColor","borderTopColor","color","outlineColor"],function(M,L){I.fx.step[L]=function(N){if(!N.colorInit){N.start=A(N.elem,L);
N.end=F(N.end);
N.colorInit=true
...
I.effects.animateClass=function(O,M,L,N){if(I.isFunction(L)){N=L;
L=null
}return this.each(function(){var T=I(this),R=T.attr("style")||" ",Q=C(D.call(this)),S,P=T.attr("className");
I.each(B,function(U,V){O[V]&&T[V+"Class"](O[V])
});
S=C(D.call(this));
T.attr("className",P);
T.animate(J(Q,S),M,L,function(){I.each(B,function(U,V){O[V]&&T[V+"Class"](O[V])
});
if(typeof T.attr("style")=="object"){T.attr("style").cssText="";
T.attr("style").cssText=R
}else{T.attr("style",R)
}N&&N.apply(this,arguments)
})
})
};
I.fn.extend({_addClass:I.fn.addClass,addClass:function(O,M,L,N){return M?I.effects.animateClass.apply(this,[{add:O},M,L,N]):this._addClass(O)
},_removeClass:I.fn.removeClass,removeClass:function(O,M,L,N){return M?I.effects.animateClass.apply(this,[{remove:O},M,L,N]):this._removeClass(O)
},_toggleClass:I.fn.toggleClass,toggleClass:function(P,M,L,O,N){return typeof M=="boolean"||M===H?L?I.effects.animateClass.apply(this,[M?{add:P}:{remove:P},L,O,N]):this._toggleClass(P,M):I.effects.animateClass.apply(this,[{toggle:P},M,L,O])
},switchClass:function(P,M,L,O,N){return I.effects.animateClass.apply(this,[{add:M,remove:P},L,O,N])
}});
I.extend(I.effects,{version:"1.8.5",save:function(N,M){for(var L=0;
L<M.length;
L++){M[L]!==null&&N.data("ec.storage."+M[L],N[0].style[M[L]])
}},restore:function(N,M){for(var L=0;
L<M.length;
L++){M[L]!==null&&N.css(M[L],N.data("ec.storage."+M[L]))
}},setMode:function(M,L){if(L=="toggle"){L=M.is(":hidden")?"show":"hide"
}return L
},getBaseline:function(N,M){var L;
switch(N[0]){case"top":L=0;
break;
case"middle":L=0.5;
break;
case"bottom":L=1;
break;
default:L=N[0]/M.height
}switch(N[1]){case"left":N=0;
break;
case"center":N=0.5;
break;
case"right":N=1;
break;
default:N=N[1]/M.width
}return{x:N,y:L}
},createWrapper:function(N){if(N.parent().is(".ui-effects-wrapper")){return N.parent()
}var M={width:N.outerWidth(true),height:N.outerHeight(true),"float":N.css("float")},L=I("<div></div>").addClass("ui-effects-wrapper").css({fontSize:"100%",background:"transparent",border:"none",margin:0,padding:0});
N.wrap(L);
L=N.parent();
if(N.css("position")=="static"){L.css({position:"relative"});
N.css({position:"relative"})
}else{I.extend(M,{position:N.css("position"),zIndex:N.css("z-index")});
I.each(["top","left","bottom","right"],function(P,O){M[O]=N.css(O);
if(isNaN(parseInt(M[O],10))){M[O]="auto"
}});
...
},setTransition:function(O,M,L,N){N=N||{};
I.each(M,function(Q,P){unit=O.cssUnit(P);
if(unit[0]>0){N[P]=unit[0]*L+unit[1]
}});
return N
}});
I.fn.extend({effect:function(N){var M=G.apply(this,arguments);
M={options:M[1],duration:M[2],callback:M[3]};
var L=I.effects[N];
return L&&!I.fx.off?L.call(this,M):this
},_show:I.fn.show,show:function(M){if(!M||typeof M=="number"||I.fx.speeds[M]||!I.effects[M]){return this._show.apply(this,arguments)
}else{var L=G.apply(this,arguments);
L[1].mode="show";
return this.effect.apply(this,L)
}},_hide:I.fn.hide,hide:function(M){if(!M||typeof M=="number"||I.fx.speeds[M]||!I.effects[M]){return this._hide.apply(this,arguments)
}else{var L=G.apply(this,arguments);
L[1].mode="hide";
return this.effect.apply(this,L)
}},__toggle:I.fn.toggle,toggle:function(M){if(!M||typeof M=="number"||I.fx.speeds[M]||!I.effects[M]||typeof M=="boolean"||I.isFunction(M)){return this.__toggle.apply(this,arguments)
}else{var L=G.apply(this,arguments);
L[1].mode="toggle";
return this.effect.apply(this,L)
}},cssUnit:function(N){var M=this.css(N),L=[];
I.each(["em","px","%","pt"],function(P,O){if(M.indexOf(O)>0){L=[parseFloat(M),O]
}});
return L
}});
I.easing.jswing=I.easing.swing;
I.extend(I.easing,{def:"easeOutQuad",swing:function(P,M,L,O,N){return I.easing[I.easing.def](P,M,L,O,N)
},easeInQuad:function(P,M,L,O,N){return O*(M/=N)*M+L
},easeOutQuad:function(P,M,L,O,N){return -O*(M/=N)*(M-2)+L
},easeInOutQuad:function(P,M,L,O,N){if((M/=N/2)<1){return O/2*M*M+L
}return -O/2*(--M*(M-2)-1)+L
},easeInCubic:function(P,M,L,O,N){return O*(M/=N)*M*M+L
},easeOutCubic:function(P,M,L,O,N){return O*((M=M/N-1)*M*M+1)+L
},easeInOutCubic:function(P,M,L,O,N){if((M/=N/2)<1){return O/2*M*M*M+L
}return O/2*((M-=2)*M*M+2)+L
},easeInQuart:function(P,M,L,O,N){return O*(M/=N)*M*M*M+L
},easeOutQuart:function(P,M,L,O,N){return -O*((M=M/N-1)*M*M*M-1)+L
},easeInOutQuart:function(P,M,L,O,N){if((M/=N/2)<1){return O/2*M*M*M*M+L
}return -O/2*((M-=2)*M*M*M-2)+L
},easeInQuint:function(P,M,L,O,N){return O*(M/=N)*M*M*M*M+L
},easeOutQuint:function(P,M,L,O,N){return O*((M=M/N-1)*M*M*M*M+1)+L
},easeInOutQuint:function(P,M,L,O,N){if((M/=N/2)<1){return O/2*M*M*M*M*M+L
}return O/2*((M-=2)*M*M*M*M+2)+L
},easeInSine:function(P,M,L,O,N){return -O*Math.cos(M/N*(Math.PI/2))+O+L
},easeOutSine:function(P,M,L,O,N){return O*Math.sin(M/N*(Math.PI/2))+L
},easeInOutSine:function(P,M,L,O,N){return -O/2*(Math.cos(Math.PI*M/N)-1)+L
},easeInExpo:function(P,M,L,O,N){return M==0?L:O*Math.pow(2,10*(M/N-1))+L
},easeOutExpo:function(P,M,L,O,N){return M==N?L+O:O*(-Math.pow(2,-10*M/N)+1)+L
},easeInOutExpo:function(P,M,L,O,N){if(M==0){return L
}if(M==N){return L+O
}if((M/=N/2)<1){return O/2*Math.pow(2,10*(M-1))+L
}return O/2*(-Math.pow(2,-10*--M)+2)+L
},easeInCirc:function(P,M,L,O,N){return -O*(Math.sqrt(1-(M/=N)*M)-1)+L
},easeOutCirc:function(P,M,L,O,N){return O*Math.sqrt(1-(M=M/N-1)*M)+L
},easeInOutCirc:function(P,M,L,O,N){if((M/=N/2)<1){return -O/2*(Math.sqrt(1-M*M)-1)+L
}return O/2*(Math.sqrt(1-(M-=2)*M)+1)+L
},easeInElastic:function(R,M,L,Q,P){R=1.70158;
var O=0,N=Q;
if(M==0){return L
}if((M/=P)==1){return L+Q
}O||(O=P*0.3);
if(N<Math.abs(Q)){N=Q;
R=O/4
}else{R=O/(2*Math.PI)*Math.asin(Q/N)
}return -(N*Math.pow(2,10*(M-=1))*Math.sin((M*P-R)*2*Math.PI/O))+L
},easeOutElastic:function(R,M,L,Q,P){R=1.70158;
var O=0,N=Q;
if(M==0){return L
}if((M/=P)==1){return L+Q
}O||(O=P*0.3);
...
}return N*Math.pow(2,-10*M)*Math.sin((M*P-R)*2*Math.PI/O)+Q+L
},easeInOutElastic:function(R,M,L,Q,P){R=1.70158;
var O=0,N=Q;
if(M==0){return L
}if((M/=P/2)==2){return L+Q
}O||(O=P*0.3*1.5);
if(N<Math.abs(Q)){N=Q;
R=O/4
}else{R=O/(2*Math.PI)*Math.asin(Q/N)
}if(M<1){return -0.5*N*Math.pow(2,10*(M-=1))*Math.sin((M*P-R)*2*Math.PI/O)+L
}return N*Math.pow(2,-10*(M-=1))*Math.sin((M*P-R)*2*Math.PI/O)*0.5+Q+L
},easeInBack:function(Q,M,L,P,O,N){if(N==H){N=1.70158
}return P*(M/=O)*M*((N+1)*M-N)+L
},easeOutBack:function(Q,M,L,P,O,N){if(N==H){N=1.70158
}return P*((M=M/O-1)*M*((N+1)*M+N)+1)+L
},easeInOutBack:function(Q,M,L,P,O,N){if(N==H){N=1.70158
}if((M/=O/2)<1){return P/2*M*M*(((N*=1.525)+1)*M-N)+L
}return P/2*((M-=2)*M*(((N*=1.525)+1)*M+N)+2)+L
},easeInBounce:function(P,M,L,O,N){return O-I.easing.easeOutBounce(P,N-M,0,O,N)+L
},easeOutBounce:function(P,M,L,O,N){return(M/=N)<1/2.75?O*7.5625*M*M+L:M<2/2.75?O*(7.5625*(M-=1.5/2.75)*M+0.75)+L:M<2.5/2.75?O*(7.5625*(M-=2.25/2.75)*M+0.9375)+L:O*(7.5625*(M-=2.625/2.75)*M+0.984375)+L
},easeInOutBounce:function(P,M,L,O,N){if(M<N/2){return I.easing.easeInBounce(P,M*2,0,O,N)*0.5+L
}return I.easing.easeOutBounce(P,M*2-N,0,O,N)*0.5+O*0.5+L
}})
}(jQuery);
...
$.extend($.ui.tabs.prototype,{paging:function(E){var M={tabsPerPage:0,nextButton:"&#187;",prevButton:"&#171;",follow:false,cycle:false};
M=jQuery.extend(M,E);
var O=this,G=false,R,Q,K,F,I,B,N,L,D=null,J=$(window).height(),V=$(window).width();
function S(){F=0,R=0,N=0,Q=0,L=new Array(),I=new Array(),B=new Array();
K=$(O.element).width();
var b=$("<li></li>").addClass("ui-tabs-paging-next");
var Z=$('<a href="#"></a>').click(function(){H("next");
return false
}).html(M.nextButton);
b.append(Z);
O.lis.eq(O.length()-1).after(b);
Q=b.outerWidth({margin:true});
b=$("<li></li>").addClass("ui-tabs-paging-prev");
Z=$('<a href="#"></a>').click(function(){H("prev");
return false
}).html(M.prevButton);
b.append(Z);
O.lis.eq(0).before(b);
Q+=b.outerWidth({margin:true});
O.lis.each(function(c){if(c==O.options.selected){B[c]=$(this).outerWidth({margin:true});
I[c]=O.lis.eq(c).removeClass("ui-tabs-selected").outerWidth({margin:true});
O.lis.eq(c).addClass("ui-tabs-selected");
F+=B[c]
}else{I[c]=$(this).outerWidth({margin:true});
B[c]=O.lis.eq(c).addClass("ui-tabs-selected").outerWidth({margin:true});
O.lis.eq(c).removeClass("ui-tabs-selected");
F+=I[c]
}});
if(F>K){var W=0,X=0,a=0;
for(var Y=0;
Y<I.length;
Y++){if(X==0||B[Y]-I[Y]>a){a=(B[Y]-I[Y])
}if(L[W]==null){L[W]={start:Y}
}else{if((Y>0&&(Y%M.tabsPerPage)==0)||(I[Y]+X+Q+12)>K){if((X+a)>N){N=(X+a)
}W++;
L[W]={start:Y};
X=0
}}L[W].end=Y+1;
X+=I[Y];
if(Y==O.options.selected){R=W
}}if((X+a)>N){N=(X+a)
}O.lis.hide().slice(L[R].start,L[R].end).show();
if(R==(L.length-1)&&!M.cycle){A("next")
}if(R==0&&!M.cycle){A("prev")
}G=true
}else{T()
}$(window).bind("resize",U)
}function H(X){R=R+(X=="prev"?-1:1);
if(X=="prev"&&R<0&&M.cycle){R=L.length-1
}else{if((X=="prev"&&R<0)||(X=="next"&&R>=L.length)){R=0
}}var Y=L[R].start;
var W=L[R].end;
O.lis.hide().slice(L[R].start,L[R].end).show();
if(X=="prev"){P("next");
if(M.follow&&(O.options.selected<Y||O.options.selected>(W-1))){O.select(W-1)
}if(!M.cycle&&Y<=0){A("prev")
}}else{P("prev");
if(M.follow&&(O.options.selected<Y||O.options.selected>(W-1))){O.select(Y)
}if(!M.cycle&&W>=O.length()){A("next")
}}}function A(W){$(".ui-tabs-paging-"+W,O.element).addClass("ui-tabs-paging-disabled")
}function P(W){$(".ui-tabs-paging-"+W,O.element).removeClass("ui-tabs-paging-disabled")
}function U(){if(D){clearTimeout(D)
}if(J!=$(window).height()||V!=$(window).width()){D=setTimeout(C,100)
}}function C(){J=$(window).height();
V=$(window).width();
T();
S()
}function T(){$(".ui-tabs-paging-next",O.element).remove();
$(".ui-tabs-paging-prev",O.element).remove();
O.lis.show();
G=false;
$(window).unbind("resize",U)
}S();
$.extend($.ui.tabs.prototype,{pagingAdd:function(Y,X,W){if(G){T();
this.add(Y,X,W);
S()
}else{this.add(Y,X,W)
}},pagingRemove:function(W){if(G){T();
this.remove(W);
S()
}else{this.remove(W)
}},pagingDestroy:function(){T()
}})
}});
...
}function m(x,k){var AA=document.createElement("ul");
var z=x.length;
...
if(T.formatItem){AB.innerHTML=T.formatItem(AC,y,z,k);
$(AB).children().each(function(){this.selectValue=AC[0]
});
AB.selectValue=AC[0]
}else{AB.innerHTML=AC[0];
AB.selectValue=AC[0]
}var v=null;
if(AC.length>1){v=[];
for(var w=1;
w<AC.length;
w=w+1){v[v.length]=AC[w]
}}AB.extra=v;
AA.appendChild(AB)
}$(AA).children("li").hover(function(){$("li",AA).removeClass("ac_over");
$(this).addClass("ac_over");
l=$("li",AA).indexOf($(this).get(0))
...
if(T.formatItem){v.innerHTML=T.formatItem(AB,y,x,AA);
$(v).find().each(function(){this.selectValue=AB[0]
});
v.selectValue=AB[0]
...
Cheatsheet.prototype.addChangeMarker=function(C,A,E){if(A){C.append(" ");
var D=$("<span></span>").addClass("attribute").addClass(A);
var B="proposed as "+A+" in HTML5 (not yet a standard)";
if(E==="full"){D.text(B)
}else{if(E==="medium"){D.text(A+" in draft of HTML5");
D.attr("title",B)
}else{D.text(A);
D.attr("title",B)
}}D.appendTo(C)
}};
Cheatsheet.prototype.show_keyword=function(T,M,V){var d=this,B="http://www.w3.org";
if(T[M][V]===null){return false
}function R(l,m,k,j){var n=$("<a class='internal'></a>").attr("href","#inf,"+escape(l)+","+escape(m)+","+escape(k));
n.text(k);
if(j){d.addChangeMarker(n,j,"short")
}return n
}for(var i in T[M][V]){var J=this.keywordSources[M][V];
var a=$("<div></div>").addClass("context");
var h=$("<h2></h2>").text(J+" ");
$("<code></code>").text(i).appendTo(h);
h.appendTo(a);
var F=$("<div></div>").appendTo(a);
for(var b in T[M][V][i]["d"]){var G=T[M][V][i]["d"][b];
var Y=$("<div></div>");
if(T[M][V][i]["d"].length>1&&G.ct&&G.ct.y){var C=$("<h3></h3>");
if(G.ct.y==="a"){C.text("With attribute ")
}else{if(G.ct.y==="e"){C.text("In element ")
}}for(var Z in G.ct.d){$("<code></code>").text(G.ct.d[Z]).appendTo(C);
if(Z<G.ct.d.length-1){C.append(", ")
}}if(G.h){d.addChangeMarker(C,G.h.p[0].t,"medium")
}C.appendTo(Y)
}else{if(G.h){d.addChangeMarker(h,G.h.p[0].t,"medium")
}}var g=$("<dl></dl>");
for(var P in G){var O=G[P];
if(P!=="ct"){var c=$("<dt></dt>").appendTo(g),U=c,X=O.p;
if(O.u){var K=O.u;
if(K.substring(0,1)==="/"){K=B+K
}U=$("<a></a>").attr("href",K).appendTo(c)
}U.text(dictionary[P]);
if(X&&X.length>0){var A=true;
if(X.length===1||O.l==="inline"){A=false
}var e=$("<dd></dd>");
if(A){if(O.l==="block"){e=$("<ul></ul>").appendTo(e)
}}for(var Q in X){var H=false,E=e,I=X[Q];
if(A){E=$("<li></li>").appendTo(E)
}else{E=$("<span></span>").appendTo(E)
}if(I.u){var L=I.u;
if(L.substring(0,1)==="/"){L=B+L
}E=$("<a></a>").attr("href",L).appendTo(E);
E.text(I.t);
H=true
}else{if(O.i&&O.y){var N=R(O.i,O.y,I.t,I.h);
E=N.appendTo(E);
H=true
}}if(I.t instanceof Array){if(!H){for(var f in I.t){var S=I.t[f];
if(S.y&&S.i&&S.t){var D=R(S.i,S.y,S.t,S.h);
D.appendTo(E)
}else{var W=document.createTextNode(S);
E.get(0).appendChild(W)
}}}else{E.text(I.t.join(""))
}}else{if(P==="h"){d.addChangeMarker(E,I.t,"full")
}else{if(!H){E.text(I.t)
}}}if(!A&&Q<X.length-1){e.append(" ")
}}e.appendTo(g)
}}}g.appendTo(Y);
Y.appendTo(F)
}a.appendTo($("#details"))
...
Cheatsheet.prototype.load_keyword_data=function(A,B,E){var D=keywordsMatch[A];
if(inMemory&&D&&(!B||(D[B]&&D[B][E]))){this.display_keyword_data(D,B,E)
}else{var C=this;
$.getJSON("data/json/"+escape(A.replace(/ /g,"%20")).toLowerCase()+".js",function(F){C.display_keyword_data(F,B,E)
})
}};
Cheatsheet.prototype.clearLookUp=function(){var A=$("#details");
if(A.data("accordion")&&A.data("accordion").element){A.accordion("destroy")
}A.empty()
};
Cheatsheet.prototype.load_anchor=function(C){var E=$("#search");
if(C===null){return false
}if(/^search,/.test(C)){E.val(C.substring(7));
E.get(0).autocompleter.findValue();
return true
}var A=C.split(",");
var D=unescape(A[1]);
var F=unescape(A[2]);
var B=unescape(A.slice(3).join(","));
if(B&&D&&F&&this.keywordSources[D]&&this.keywordSources[D][F]&&keywordsMatch[B]&&keywordsMatch[B][D]&&keywordsMatch[B][D][F]){this.clearLookUp();
$(".ac_results").hide();
E.val("");
if(this.load_keyword_data(B,D,F)){return true
}}return false
};
var cheatsheet=new Cheatsheet();
jQuery(document).ready(function(C){var F=cheatsheet,D=C("#content"),B=window.location.hash;
D.tabs();
D.tabs("paging",{nextButton:"",prevButton:""});
D.bind("tabsselect",function(){C(".ac_results").hide()
});
D.bind("tabsshow",function(J,K){window.location.hash=K.tab.hash
});
C(".accordion").accordion({header:"div >h3",active:false,autoHeight:false});
F.makeReplacingAccordion(C(".accordion"));
C("body").css("display","block");
function A(){var J=C("#about");
if(J.css("display")==="block"){J.css("display","none")
}else{J.css("display","block")
}return false
}C("#openabout, #closeabout, #closeabout2").click(A);
function G(){if(F.hashHistory.length>0){C("<a class='internal back' onclick='cheatsheet.hashHistory.pop();return true;'></a>").attr("href",F.hashHistory[F.hashHistory.length-1]).text("back").appendTo(C("<p></p>")).appendTo(C("#details"))
}}function H(K){if(K===null){return 
}var J=K.selectValue;
window.location.hash="#search,"+escape(J);
F.clearLookUp();
F.load_keyword_data(J)
}C("a.internal").live("click",function(){F.load_anchor(C(this).attr("href").split("#")[1]);
if(!C(this).hasClass("back")){F.hashHistory.push(window.location.hash)
}G()
});
function E(Q,M,L,P){var K=Q[0],O=K.toLowerCase().indexOf(P),J=keywordsTags[Q[0]];
K=K.substring(0,O)+"<strong>"+K.substring(O,O+P.length)+"</strong>"+K.substring(O+P.length);
if(J){var N=C("<span></span>").html(K);
F.addChangeMarker(N,J,"medium");
K=N.html()
}return K
}C("#search").autocompleteArray(keywords,{onItemSelect:H,onFindValue:H,autoFill:false,selectFirst:true,delay:40,maxItemsToShow:10,matchContains:true,matchSubset:true,formatItem:E});
var I=C("#search");
I.change(function(){var J=C("#details_clear");
F.clearLookUp();
if(I.val()){if(J.length===0){C("<a href='#' class='ui-icon-close' id='details_clear' title='Clear search'>x</a>").insertAfter(I).click(function(){F.clearLookUp();
I.val("").change();
return false
})
}}else{J.remove()
}});
if(B){if(/^#inf,/.test(B)||/^#search,/.test(B)){F.load_anchor(B.substring(1))
}}});