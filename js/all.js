...
E("#content").bind("tabsshow",function(H,I){window.location.hash=I.tab.hash
});
...
if(getCookie("alreadyLaunched")===""&&startWithDonate){G();
setCookie("alreadyLaunched","true",2000)
}E("#openabout").click(G);
function F(){E("#about").css("display","none");
return false
}function G(){if(E("#about").css("display")==="block"){F()
}else{E("#about").css("display","block");
E("#closeabout").click(F);
E("#closeabout2").click(F)
}return false
}function D(d,Q,U){if(d===null){return false
}var N=keywordSources[Q][U];
var Y=E("<div></div>").addClass("context");
E("<code></code>").text(d).appendTo(E("<h2></h2>").text(N+" ").appendTo(Y));
var J=E("<div></div>").appendTo(Y);
for(var X in keywordsMatch[d][Q][U]){var K=keywordsMatch[d][Q][U][X];
var c=E("<dl></dl>");
for(var R in K){var Z=E("<dt></dt>").appendTo(c);
var V=Z;
if(K[R].u){var O=K[R].u;
if(O.substring(0,1)==="/"){O="http://www.w3.org"+O
}V=E("<a></a>").attr("href",O).appendTo(Z)
}V.text(dictionary[R]);
if(K[R]["p"]&&K[R]["p"].length>0){var H=true;
if(K[R]["p"].length===1||K[R].l==="inline"){H=false
}var a=E("<dd></dd>");
if(H){if(K[R].l==="block"){a=E("<ul></ul>").appendTo(a)
}}for(var S in K[R]["p"]){var L=false;
var I=a;
var M=K[R]["p"][S];
if(H){I=E("<li></li>").appendTo(I)
}else{I=E("<span></span>").appendTo(I)
}if(M.u){var P=M.u;
if(P.substring(0,1)==="/"){P="http://www.w3.org"+P
}I=E("<a></a>").attr("href",P).appendTo(I);
L=true
}else{if(K[R].i&&K[R].y){I=E("<a class='internal'></a>").attr("href","#inf,"+escape(K[R].i)+","+escape(K[R].y)+","+escape(M.t)).appendTo(I);
L=true
}}if(M.t instanceof Array){if(!L){for(var b in M.t){var T=M.t[b];
if(T.y&&T.i&&T.t){E("<a class='internal'></a>").attr("href","#inf,"+escape(T.i)+","+escape(T.y)+","+escape(T.t)).text(T.t).appendTo(I)
}else{var W=document.createTextNode(T);
I.get(0).appendChild(W)
}}}else{I.text(M.t.join(""))
}}else{I.text(M.t)
}if(!H&&S<K[R]["p"].length-1){a.append(", ")
}}a.appendTo(c)
}}c.appendTo(J)
}Y.appendTo(E("#details"));
return true
...
}}function C(J){if(J===null){return false
}if(J.substring(0,7)==="search,"){E("#search").val(J.substring(7));
E("#search").get(0).autocompleter.findValue();
return true
}var H=J.split(",");
var K=unescape(H[1]);
var L=unescape(H[2]);
var I=unescape(H.slice(3).join(","));
if(I&&K&&L&&keywordSources[K]&&keywordSources[K][L]&&keywordsMatch[I]&&keywordsMatch[I][K]&&keywordsMatch[I][K][L]){clearLookUp();
E("#search").val("");
if(D(I,K,L)){E("#details").accordion({header:"div>h2",autoHeight:false});
return true
}}return false
}function A(K){if(K===null){return 
}var I=K.selectValue;
window.location.hash="#search,"+escape(I);
clearLookUp();
var H=0;
for(var J in keywordsMatch[I]){for(var L in keywordsMatch[I][J]){H=H+1;
D(I,J,L)
}}if(H===1){E("#details").accordion({header:"div>h2",autoHeight:false})
...
if(E("#search").val()){if(!E("#details_clear").length){E("#search").after("<a href='#' class='ui-icon-close' id='details_clear' title='Clear search'>x</a>");
